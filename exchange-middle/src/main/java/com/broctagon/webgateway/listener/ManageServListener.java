...
import com.broctagon.webgateway.config.WebSocketConfig;
		...
		switch (Integer.parseInt(tag)) {
			case Tag.LOGIN_RESPONSE:
				...
				if(Integer.parseInt(MessageUtil.getContentFromMsg(message, Constant.STATUS)) == 0) {
					PushService.addSessionId(MessageUtil.getContentFromMsg(message, Constant.USER_ID), sessionId);
				...
				break;
			case Tag.REGISTRY_RESPONSE://注册响应
			...
			case Tag.DELETE_BANK_ACCOUNT_RESPONSE:
				...
				break;
			case Tag.FORGET_PWD_RESPONSE://忘记密码回显
			...
			case Tag.USER_KYC_INFO_RESPONSE:
				...
				break;
			case Tag.SEND_EMAIL_RESPONSE://邮箱验证码响应
				messageSender.convertAndSend(Path.PREFIX + Path.USER_EMAIL_INFO + sessionId, message);
				break;
			default:
				break;
			}
